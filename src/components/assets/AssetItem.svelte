...
<script>
  ...
  $: pledgeMaintenanceRate = hasPledge && twdValue > 0
    ? (twdValue / pledgeBorrowed) * 100
    : 0;
  ...
  function handleDelete(holding) {
    ...
    if (sym && pledgeBorrowed > 0) {
      // 計算刪除後的剩餘市值
      const holdingValue = calculateAssetTWD(holding.quantity, holding.pricePerUnit, holding.currency, exchangeRate);
      const remainingValue = twdValue - holdingValue;
